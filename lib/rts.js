...
/* IE9 does not not support ArrayBuffer */
var decodeFloat = undefined;
var decodeDouble = undefined;
if (typeof ArrayBuffer == 'undefined') {

    /* IE9, inefficient, poorly tested, and probably wrong */

    var log2 = function (x) { return Math.log(x) / Math.log(2); };

    decodeFloat = function(x) {
        var a = x;
        var sign = x < 0 ? -1 : 1;
        var exp = Math.floor(log2(a)) - 23;
        var man = a / Math.pow(2,exp);
        return [0, sign*man, exp];
    };

    decodeDouble = function(x) {
        if (x === 0) {
            return [0, 1, 1048576, 0, -1075];
        } else {
            var a = x;
            var sign = x < 0 ? -1 : 1;
            var exp = Math.floor(log2(a)) - 52;
            var man = a / Math.pow(2,exp);
            var manLow = Math.abs(man & 0x7fffffff) + Math.abs((man & 0x80000000))
            var manHigh = Math.floor(man / Math.pow(2,32));
